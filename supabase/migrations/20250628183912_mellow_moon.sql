...
This migration creates a comprehensive food delivery platform database with the following features:

## 1. Core Tables
- **users**: Extended user profiles with user types (customer, restaurant, delivery)
- **categories**: Food categories with emojis
- **restaurants**: Restaurant information with ratings and delivery details
- **menu_items**: Restaurant menu items with pricing and categories
- **orders**: Order management with status tracking
- **order_items**: Individual items within orders
- **delivery_drivers**: Driver profiles and availability
- **deliveries**: Delivery tracking and management

## 2. Security
- Row Level Security (RLS) enabled on all tables
- Proper access control policies for each user type
...
## 4. Sample Data
- 10 food categories with emojis
...
-- Insert sample restaurants only if none exist
DO $$
BEGIN
...
BEGIN
    ...
    IF mario_restaurant_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = mario_restaurant_id LIMIT 1) THEN
...
BEGIN
    ...
    -- Burger Station menu
    IF burger_restaurant_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = burger_restaurant_id LIMIT 1) THEN
        INSERT INTO menu_items (restaurant_id, name, description, price, image, category, popular) VALUES
    ...
    -- Sushi Zen menu
    IF sushi_restaurant_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = sushi_restaurant_id LIMIT 1) THEN
        INSERT INTO menu_items (restaurant_id, name, description, price, image, category, popular) VALUES
